...
public interface MoodRepository extends JpaRepository<Mood, Long> {
    ...
    List<Mood> findByUserAndDateBetween(User user, LocalDate start, LocalDate end);

    @Query("SELECT AVG(m.intensity) FROM Mood m WHERE m.user = :user AND m.date BETWEEN :start AND :end")
    Double getAverageIntensity(@Param("user") User user, @Param("start") LocalDate start, @Param("end") LocalDate end);
